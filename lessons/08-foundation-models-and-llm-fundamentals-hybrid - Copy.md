...
| Tools | Python, pytest, Pydantic, PyTorch, Hugging Face Transformers, tokenizer visualizers, optional Jupyter, FastAPI, Typer, Docker, OpenTelemetry, and a hosted model API only for manually recorded comparison results. |
...
- Jupyter is optional for controlled experiment notes; scripts and tests remain the source of truth.
- A hosted model account is optional for manual comparison only. Use synthetic data and record provider policy details if you use one.
...
| Tokenizer visual inspection | Hugging Face tokenizer output plus optional tokenizer visualizers | Makes model-specific token boundaries visible | Visualizers do not replace reproducible token reports | Provider tokenizer UI or notebook display |
| Controlled experiment notes | Optional Jupyter notebook | Useful for inspecting tokenization, probability tables, and manual hosted outputs | Notebook output is not the source of truth | CLI scripts, JSONL records, and pytest |
| Manual hosted comparison | Hosted model API result recorded by hand | Lets learners compare hosted behavior before provider integration | Not automated, not a gateway, not production evidence | Lesson 09 provider integration |
...
Test case: Compare `A-1049`, JSON, an email thread, a Markdown table, copied PDF/policy text, non-English text, and emoji with the target tokenizer.
...
- Guided: Inspect token counts for English prose, an email thread, JSON, a product code, Markdown, copied PDF/policy text, and multilingual text. Explain which input is most expensive and why.
...
| Base vs instruction model | Same support cases across a comparable base checkpoint and instruction/chat-tuned checkpoint | raw continuation prompt for the base model; documented chat template for the instruction model | instruction following, missing-evidence handling, unsafe claims | Instruction-tuned model follows task boundaries better | Base model continues prompt, ignores role intent, or invents missing evidence |
...
Concrete base-versus-instruction setup: choose two comparable candidates from the same model family when your license, hardware, or provider access allows it. One should be a base continuation model; the other should be an instruction/chat-tuned sibling or equivalent hosted candidate. Record the exact model IDs, revisions, prompt format, and chat template. If you can only access the instruction model through a hosted model API, use the recorded hosted-model result format below and mark the comparison as manual.

## Recorded hosted-model result format

Lesson 08 does not implement provider APIs. That belongs to Lesson 09. The hosted model API path in this lesson is manual only: if you test a hosted model using synthetic data, record the result in the same evidence style:
...
```json
{
  ...
  "provider": {
    ...
    "regional_availability_checked": true,
    "provider_data_policy_reviewed": true,
    "data_retention_policy_reviewed": true,
    "training_on_inputs_policy_reviewed": true,
    "enterprise_privacy_mode": "enabled-or-not-applicable",
    "policy_notes": "Only synthetic data was used."
  },
  "settings": {
    "temperature": 0.2,
...
- Record provider region, regional availability, provider data policy, retention policy, and training-on-input policy.
- Record whether an enterprise/privacy mode was enabled or not applicable.
...
Completion evidence for this lesson is a fixed test set, reproducible generation settings, a model-selection report, and no unsupported benchmark claims. If a result was not measured in your lab, write "not measured" instead of implying it is known.
...
Provider or serving location: Local open-weight run, no hosted region.
Regional availability: Not applicable for the local run; required before hosted selection.
Provider data policy: Not applicable for the local run; required before hosted selection.
...
| Output variability hides a regression | Sampling variation mixed with model-version changes | Run records show changed seed, settings, or model revision | Re-run fixed deterministic cases before comparing sampled outputs | Keep reproducible generation settings separate from variation tests |
...
| Unexpected token cost appears after adding context | Policy text, copied PDF text, chat template, or JSON expands token count | Tokenizer report and input-token totals | Shorten or restructure context in later lessons | Measure token count before each experiment |
| Context overflow or truncation removes evidence | Input exceeds max context or local cap | Token report near/exceeds case limit; output ignores policy | Reduce input or defer retrieval/context engineering to later lessons | Set max-input-token checks and fail early |
...
| Model-version changes alter behavior | Unpinned revision, provider snapshot change, or local cache update | Result record lacks revision or date | Pin revision when possible; re-run fixed set | Record model ID, revision, provider, and run date |
...
| Hosted privacy | Do not send real customer data in this lesson; review provider data policy before business use. |
| Hosted region and retention | For manually recorded hosted results, record serving region, regional availability, retention policy, training-on-input policy, and enterprise/privacy mode. |
...
- [ ] Hosted runs, if any, record provider region and regional availability.
- [ ] Hosted runs, if any, record provider data policy, retention policy, training-on-input policy, and privacy mode.
